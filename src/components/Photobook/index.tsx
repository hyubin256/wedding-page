...
const Photobook: React.FC = () => {
  return (
    <div className="max-w-5xl mx-auto py-20 flex flex-col gap-20 text-black">
      <h2 className="uppercase text-center text-4xl font-bold">
        Photobook
      </h2>
      <div className="grid grid-cols-4 gap-5">
        {
          listOfImage.map((image, index) => {
            return (
              <div
                key={`photobook-${index}`}
                className={clsx((index + 1) % 6 === 1 || (index + 1) % 6 === 0 ? "col-span-2" : "col-span-1", "h-[400px]")}
              >
